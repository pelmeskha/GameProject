...
using GameProject.SceneUtilities: init_input, update_scene!, display_object, generate_background, display_background!, remove_object_from_scene, big_tank_shot, spawn_straight_propelling
using GameProject.Objects

# Создание сцены и отрисовка изображения
fig = Figure(; size = (1920, 1080), fps=144)
N_x, N_y = 200, 200
ax = fig[1, 1] = Axis(fig, aspect=DataAspect(), limits = (-N_x/2, N_x/2, -N_y/2, N_y/2))
deactivate_interaction!(ax, :rectanglezoom)

image = generate_background(N_x, N_y, 300; min_color=(1.0, 1.0, 1.0), max_color=(0.7, 0.8, 0.7))
